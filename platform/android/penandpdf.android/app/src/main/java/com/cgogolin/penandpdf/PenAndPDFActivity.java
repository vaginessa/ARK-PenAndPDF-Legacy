...
public class PenAndPDFActivity extends AppCompatActivity implements SharedPreferences.OnSharedPreferenceChangeListener, SearchView.OnQueryTextListener, SearchView.OnCloseListener, FilePicker.FilePickerSupport, TemporaryUriPermission.TemporaryUriPermissionProvider {
	...
	protected void onResume() {
		...
		if (android.os.Build.VERSION.SDK_INT >= 23 && (checkSelfPermission(android.Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED || checkSelfPermission(android.Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED)) {
	...
	public boolean onOptionsItemSelected(MenuItem item) { //Handel clicks in the options menu
		switch (item.getItemId()) {
			...
			case R.id.menu_delete_note:
				...
				return true;
			case R.id.menu_save:
				if (android.os.Build.VERSION.SDK_INT >= 23 && (checkSelfPermission(android.Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED))
	...
	public void setupCore() {//Called during onResume()
		if (core == null) {
			...
			try {
				...
				if (core == null)
					throw new Exception(getResources().getString(R.string.unable_to_interpret_uri) + " " + uri);
			} catch (Exception e) {
				AlertDialog alert = mAlertBuilder.create();
...
//                    try{Thread.sleep(2000);}catch(Exception e){}//ONLY FOR DEBUGGING REMOVE THIS!
			...
			protected void onPostExecute(Exception result) {
				if (waitWhileSavingDialog != null && waitWhileSavingDialog.isShowing())
					...
					}//This throws an IllegalArgumentException if the app has been stopped while we were saving.
				if (result == null) {
